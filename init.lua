...
require('nixCatsUtils.lazyCat').setup(nixCats.pawsible { 'allPlugins', 'start', 'lazy.nvim' }, {
  ...
  { -- LSP Configuration & Plugins
    ...
    dependencies = {
      -- blink.cmp adds extra completion capabilities for LSP
      'saghen/blink.cmp',
    ...
    config = function()
      ...
      --  When you add a completion plugin, Neovim now has *more* capabilities.
      --
      -- With blink.cmp, we create new capabilities, then broadcast that to the servers.
  ...
  { -- Autocompletion
    -- blink.cmp is a batteries-included completion engine.
    -- It works out of the box with LSP, path, snippets and buffer sources.
    --
    -- If you're coming from nvim-cmp:
    -- - the basic workflow is the same (menu + docs + accept)
    -- - keymaps are configured in `opts.keymap`
    -- - sources are configured in `opts.sources`
    'saghen/blink.cmp',
    name = 'blink.cmp',
    lazy = false,
    dependencies = {
      ...
      -- Optional: provides extra snippets for the `snippets` source.
    ...
    opts = {
      ...
      keymap = {
        ...
        -- Keep the old "jump in snippets" keys from the nvim-cmp config.
        ['<C-l>'] = { 'snippet_forward', 'fallback' },
        ['<C-h>'] = { 'snippet_backward', 'fallback' },
      },

      -- Tell blink to use Luasnip for expanding/jumping.
      snippets = { preset = 'luasnip' },

      completion = { documentation = { auto_show = false } },

      -- Sources:
      -- - `lsp`, `path`, `snippets`, `buffer` are built-in.
      -- - We also register an orgmode provider so Org buffers get org-specific completions.
      sources = {
      ...
      -- Rust fuzzy matcher when available, otherwise fallback.
